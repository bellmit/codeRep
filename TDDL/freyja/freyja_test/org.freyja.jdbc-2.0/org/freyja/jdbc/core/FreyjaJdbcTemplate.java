...
public class FreyjaJdbcTemplate extends FreyjaJdbcAccessor implements
	...
	private <T> T realSave(T t) {
		...
		Assert.notNull(t);
		ShardingEntity<T> entity = ShardingUtil.getEntity(t.getClass());
		...
		try {
			if (pd.getPropertyType() == Long.class) {
				method.invoke(t, id.longValue());
			} else {
				method.invoke(t, id.intValue());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public <T> void batchSave(final List<T> list) {
		...
		int[] sqlTypes = parameter.getSqlTypes();
